...
for i, xi in enumerate(x):
    ...
    if y_expected == 0:  # combustion
        if dist < 30:
            ...
            combustion_total[3] += 1
        elif 200 <= dist:
            combustion_total[4] += 1
...
result_combustion = [combustion_hit[i]/combustion_total[i] for i in range(5)]
result_electric = [electric_hit[i]/electric_total[i] for i in range(5)]

fig = plt.figure()
ax = fig.add_subplot(111)

ind = range(5)                # the x locations for the groups

width = 0.4                      # the width of the bars

rects1 = ax.bar(ind, result_combustion, width, color="gray")

#ax.set_xlim(-width,len(ind)+width)
#ax.set_ylim(min(result_combustion)-1,max(result_combustion)+1)
ax.set_ylabel('Proporción de acierto', fontsize=12)
ax.set_xlabel('Agrupación de tramos por intervalo de longitudes (metros) y número de tramos', fontsize=12)
#ax.set_title('Aciertos de la red neuronal para los tramos de combustión según su longitud.',fontsize=14)

xTickMarks = ["<30 ("+str(combustion_total[0])+")", "30-80 ("+str(combustion_total[1])+")", "80-150 ("+str(combustion_total[2])+")", "150-200 ("+str(combustion_total[3])+")", ">200 ("+str(combustion_total[4])+")"]
ax.set_xticks(ind)
